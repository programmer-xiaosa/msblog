...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      try {
        const res = await getCategoryList(this.pages)
      ...
      this.$refs.form.validate(async(valid) => {
        if (valid) {
          ...
          try {
            if (this.form.id) {
              try {
                const res = await updateCategory(formatForm)
                if (res.errCode === 0) {
                  this.$message({
                    message: '修改成功',
                    type: 'success'
              ...
              try {
                const res = await addCategory(formatForm)
                if (res.errCode === 0) {
                  this.$message({
                    message: '新增成功',
                    type: 'success'
    ...
    async deleteHandle(id) {
      try {
        await this.$confirm('是否确认删除分类编号为"' + id + '"的数据项?', '警告', {
