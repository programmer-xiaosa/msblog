...
<script>
import { donwloadTagsTemplate } from '@/api/tags'
...
export default {
  ...
  methods: {
    ...
    async downLoadHandle() {
      try {
        const result = await donwloadTagsTemplate()
        // 调用下载方法
        download(result, '标签数据导入模板' + getNowFormatDate() + '.xlsx') // 文件名的格式，一般是确定的，如果是excel的话就 .xls 或者 .xlsx 其中一个
      } catch (error) {
        console.log(error)
      }
    }
